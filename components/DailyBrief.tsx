...
const LANG_TO_SIGNAL: Record<string, { label: string; color: string }> = {
  TypeScript:  { label: 'SaaS/Web', color: '#3178C6' },
  JavaScript:  { label: 'SaaS/Web', color: '#F59E0B' },
  Python:      { label: 'AI应用',   color: '#7C3AED' },
  Jupyter:     { label: 'AI应用',   color: '#7C3AED' },
  Go:          { label: '基础设施', color: '#059669' },
  Rust:        { label: '基础设施', color: '#DC2626' },
  C:           { label: '基础设施', color: '#6B7280' },
  'C++':       { label: '基础设施', color: '#6B7280' },
  Swift:       { label: '移动端',   color: '#FA7343' },
  Kotlin:      { label: '移动端',   color: '#A97BFF' },
  Dart:        { label: '移动端',   color: '#00B4AB' },
  Ruby:        { label: 'Web工具',  color: '#CC342D' },
  PHP:         { label: 'Web工具',  color: '#4F5D95' },
  Java:        { label: '企业软件', color: '#B07219' },
  Shell:       { label: '开发工具', color: '#4EAA25' },
}
    ...
    <div className="mb-8 print:hidden">
      ...
      >
        ...
        >
          ...
          <p className="font-sans mb-3" style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>
            这些项目正在被大量开发者关注，代表近期技术趋势，是副业选品和方向判断的早期信号。
          </p>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {repos.map(repo => {
                const signal = repo.language ? LANG_TO_SIGNAL[repo.language] : undefined
                return (
                  <a
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        ...
                        {signal && (
                          <span
                            className="flex-shrink-0 font-mono rounded px-1.5 py-0.5"
                            style={{ fontSize: '9px', background: `${signal.color}33`, color: signal.color, border: `1px solid ${signal.color}55` }}
                          >
                            {signal.label}
                          </span>
                        )}
                      ...
                      {repo.description && (
                        <p
                          className="font-sans leading-snug line-clamp-1"
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        ...
                        <span className="flex items-center gap-0.5" style={{ color: 'rgba(255,255,255,0.4)' }}>
                          ...
                          <span className="font-mono" style={{ fontSize: '9px' }}>{repo.starsToday.toLocaleString()}</span>
                        </span>
                      </div>
                    </div>
                  </a>
